...
/// Each page is allocated via map_alloc and written via AddrSpace::write.
pub fn load_vm_image(fname: &str, uspace: &mut AddrSpace) -> axio::Result<()> {
    ...
    loop {
        ...
        uspace
            .map_alloc(va.into(), PAGE_SIZE_4K, flags, true)
            .map_err(|_| axio::Error::NoMemory)?;

        // Write data into the mapped page
        uspace
            .write(va.into(), &buf[..n])
            .map_err(|_| axio::Error::NoMemory)?;
        ...
        #[cfg(target_arch = "aarch64")]
        {
            ...
            let cache_va = axhal::mem::phys_to_virt(paddr).as_usize();
            unsafe {
                let mut off = 0usize;
